port module Game.Main exposing (main)
...
import AFrame.Core exposing (Vec3, id, scene, entity)
...
update msg model =
    case Debug.log "msg" msg of
        BallLoaded body ->
            ...
            , resetBall { id = ids.ball, props = ballStart }
            )

        PaddleCollided collision ->
            if model.status == Active && collision.body.id == model.ballBodyId then
                ( { model
                    | score = model.score + 1
                    , status = Resetting
                  }
...
port resetBall : { id : String, props : BallProps } -> Cmd msg
